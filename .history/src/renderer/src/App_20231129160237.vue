...
const recording = ref(new Guacamole.Recording())
const guacOnPause = () => {
  // 播放
  console.log('播放')
  playButton.value = '播放'
...
const playGuacd = (video: VideoInfo): void => {
  ...
  recording.value = new Guacamole.SessionRecording(tunnel)
  const recordingDisplay = recording.value.getDisplay()
  ...
  recording.value.connect()
  // If playing, the play/pause button should read "Pause"
  recording.value.onplay = guacOnPlay
  // If paused, the play/pause button should read "Play"
  recording.onpause = guacOnPause
  // Toggle play/pause when display or button are clicked
  displayPlayBack.value.onclick = guacClick
