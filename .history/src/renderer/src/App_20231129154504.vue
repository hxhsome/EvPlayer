...
const playGuacd = (video: VideoInfo): void => {
  ...
  this.title = `回放 ${rowData.loginName}【${rowData.mode}_${rowData.account}@${rowData.resIp}_${rowData.sessionLoginouttime}】`
  this.$nextTick(() => {
    this.transferData = rowData
    this.searchHistoryDetailedDataList({ sessionId: this.transferData.sessionId })
    //   const RECORDING_URL = 'http://192.168.186.92:8008/webterminal/read/1738043334845264453-1657527289243-ssh.guac';
    const RECORDING_URL = `${rowData.guaclogPath}/read/${rowData.sessionId}.guac`
    ...
    recordingDisplay = recording.getDisplay()
